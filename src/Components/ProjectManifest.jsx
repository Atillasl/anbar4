...
const ProjectManifest = ({ items, onUpdate, onEditItem }) => {
        ...
        <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
          {items?.map(item => (
            <tr key={item.id} className="hover:bg-indigo-50/40 dark:hover:bg-white/5 transition-colors">
              ...
              <td className="p-6 text-center italic font-black text-xs text-slate-900 dark:text-slate-200">
              ...
              <td className="p-6 font-black italic text-slate-900 dark:text-white">{item.total} ₼</td>
              <td className="p-6 text-right flex gap-3 justify-end">
                <button onClick={() => onEditItem(item)} className="text-slate-400 dark:text-slate-500 hover:text-blue-500 dark:hover:text-blue-400"><Edit3 size={18}/></button>
                <button onClick={() => onUpdate(items.filter(i => i.id !== item.id))} className="text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400"><Trash2 size={18}/></button>
              </td>
            </tr>
          ))}
        </tbody>
